...
* The S-DOT Language
We have a similiar presentation with the original [[https://graphviz.gitlab.io/_pages/doc/info/lang.html][DOT Language]], but in a single list.
...
- The s-expression has the synat of the S-DOT Language.
...
- a keyword ~dot-exe~ for the target dot application, it is ~dot~ defaultly.
- a keyword ~dot-arguments~ for the additional dot arguments, it is null defaultly.
...
* Examples
** Preparation
We will store all images in this section in the subdirection ~images~ 
#+BEGIN_SRC lisp :load no
